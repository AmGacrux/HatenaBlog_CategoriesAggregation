...
function doGet(e) {
  /* 累計データの取得 */
  var data = getData('1q71rlMfLyypae9tTDt-ZA8qs4PJdlH9iKaMdZHWSeks', 'article_data');
  ...
  /* 現在データの取得 */
  var hatena_id = PropertiesService.getScriptProperties().getProperty('hatena_id');
  var blog_id = PropertiesService.getScriptProperties().getProperty('blog_id');
    var password = PropertiesService.getScriptProperties().getProperty('password');
  Logger.log(hatena_id);
  
  var headers = 'X-WSSE:' + createWsseAuthText(hatena_id, password);//, 'content-type': 'application/xml'};
  var url = "https://blog.hatena.ne.jp/" + hatena_id + "/" + blog_id + "/atom/entry";
  
  //var auth = { hatena_id, password };
    var urlFetchOption = {
    'method' : 'GET',    
  ...
  Logger.log(headers);
  Logger.log(response);
              Logger.log(createWsseAuthText(hatena_id, password));
  
  // outputのタプルを返す
  return output;
}

function createWsseAuthText(user_id, password) {
  var createDate = Utilities.formatDate(new Date(), 'Asia/Tokyo', 'yyyy-MM-dd').toString();
  var tmpTime = Utilities.formatDate(new Date(), 'Asia/Tokyo', 'HH:mm:ss').toString();
  createDate = createDate+'T'+tmpTime+'Z';
  var nonce = ''
  var shaObj = new jsSHA('SHA-256', 'TEXT');
  shaObj.setHMACKey(Date.now().toString(), "B64");
  shaObj.update(nonce);
  
  nonce = shaObj.getHMAC('B64');
  Logger.log(nonce);
 // var shaObj = new jsSHA('SHA-1' 'TEXT');
  ////shaObj.update(' 
                       
  // トークン生成
  var token = nonce + createDate + PropertiesService.getScriptProperties().getProperty('hatena_id');
  shaObj.update(token);
  var passwordDigest = shaObj.getHMAC('B64');
  
  return 'UsernameToken Username="'+user_id+'", PasswordDigest="'+passwordDigest+'", Nonce="'+nonce+'", Created="'+createDate+'"';
}  
...
データ比較処理：
*/
function compareCurrentPostsWithSavedPosts() {
  // hoge
  // hoge
  // hoge
  // hoge
